@echo off
chcp 65001 > nul
title WashControl — Быстрая сборка Portable

echo.
echo ╔══════════════════════════════════════════════════════╗
echo ║     WashControl — Быстрая сборка Portable .exe       ║
echo ║        Одна кнопка — готовое приложение              ║
echo ╚══════════════════════════════════════════════════════╝
echo.
...
if errorlevel 1 (
    ...
    echo  Для установки пакетов могут потребоваться права.
    echo.
...
echo.
echo  [3/3] Запускаем быструю сборку...
echo  ────────────────────────────────────────────────────────
echo.
...
:: ── Предложить открыть папку build ──────────────────────────────────────────
if exist "build\washcontrol_portable\" (
    echo  Portable версия находится в папке: %CD%\build\washcontrol_portable\
    echo.
    echo  [ИНСТРУКЦИЯ]
    echo  1. Скопируйте всю папку washcontrol_portable куда нужно
    echo  2. Запустите ЗАПУСТИТЬ.bat или washcontrol.exe
    echo  3. Откройте браузер: http://localhost:8000
    echo.
    choice /C YN /M "  Открыть папку с приложением в Проводнике?"
    if errorlevel 1 if not errorlevel 2 (
        explorer "%CD%\build\washcontrol_portable"
    )
) else (
    echo  Папка build не найдена — проверьте вывод выше.
)
