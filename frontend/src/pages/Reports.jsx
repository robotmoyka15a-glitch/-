...
const CHART_COLORS = ['#3b82f6', '#fbbf24', '#a78bfa', '#38bdf8', '#f87171', '#34d399']
...
function DayReport({ report }) {
  ...
  const tt = { background: '#1e293b', border: '1px solid #1a3a25', borderRadius: 8, color: '#f8fafc' }
        ...
        <div style={s.section}>
          ...
          {report.shifts.map(sh => (
            <div key={sh.id} style={s.shiftRow}>
              <span style={{ color:'#f8fafc' }}>{sh.full_name}</span>
              ...
              <tbody>
                {report.cars.map((c,i) => (
                  <tr key={c.id} style={{ background: i%2===0 ? '#0f172a':'#1e293b' }}>
                    ...
                    <td style={{ ...s.td, color:'#3b82f6', fontWeight:600 }}>{c.amount} ₽</td>
                    <td style={s.td}>{c.extra_service ? `⭐ ${c.extra_service_name||''}` : '—'}</td>
                    <td style={{ ...s.td, textAlign:'center' }}>
                      {c.windows_wiped ? <span style={{ color:'#3b82f6' }}>✓</span> : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function Stat({ icon, label, value, color='#f8fafc' }) {
  return (
...
const s = {
  ...
  excelBtn:    { background:'#3b82f6', color:'#0f172a', border:'none', borderRadius:8, padding:'9px 18px', cursor:'pointer', fontSize:13, fontWeight:700 },
  ...
  input:       { background:'#1e293b', border:'1px solid #1a3a25', borderRadius:8, padding:'8px 12px', color:'#f8fafc', fontSize:13, outline:'none' },
  loadBtn:     { background:'#3b82f6', color:'#0f172a', border:'none', borderRadius:8, padding:'8px 16px', cursor:'pointer', fontSize:13, fontWeight:700 },
  ...
  statCard:    { background:'#1e293b', border:'1px solid #1a3a25', borderRadius:10, padding:'14px 16px', display:'flex', flexDirection:'column', gap:4 },
  ...
  chartBox:    { background:'#1e293b', border:'1px solid #1a3a25', borderRadius:12, padding:'16px 20px' },
  ...
  th:          { padding:'8px 10px', textAlign:'left', color:'#64748b', borderBottom:'1px solid #1a3a25', whiteSpace:'nowrap' },
  td:          { padding:'7px 10px', color:'#94a3b8', borderBottom:'1px solid #1a3a25', whiteSpace:'nowrap' },
}
